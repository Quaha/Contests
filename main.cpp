...
template <typename T>
using v4 = vector<vector<vector<vector<T>>>>;
template <typename T>
using v5 = vector<vector<vector<vector<vector<T>>>>>;
...
template <typename T>
T summed(const unordered_set<T>& st) {
    T result = static_cast<T>(0);
    for (const auto& V : st) {
        result += V;
    }
...
template <typename T>
T max(const set<T>& st) {
    return *(st.rbegin());
}

template <typename T>
T max(const unordered_set<T>& st) {
    T result = *st.begin();
    for (const auto& V : st) {
        result = max(result, V);
    }
...
template <typename T>
T min(const vector<vector<T>>& arr) {
    T result = min(arr[0]);
    for (size_t i = 1; i < arr.size(); ++i) {
...
template <typename T>
T min(const set<T>& st) {
    return *(st.begin());
}

template <typename T>
T min(const unordered_set<T>& st) {
    T result = *st.begin();
    for (const auto& V : st) {
        result = min(result, V);
    }
...
void usort(T& obj) {
    sort(all(obj));
    obj.resize(unique(all(obj)) - obj.begin());
}
...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        int lowest_left = INF;
        int lowest_right = INF;

        for (auto it = poses.lower_bound(make_pair(l, 0)); it != poses.end() && (it->first <= r); ++it) {
            auto [p, cost] = *it;

            if (cost + p - (l - 1) < lowest_left) {
                lowest_left = cost + p - (l - 1);
            }

            if (cost + r + 1 - p < lowest_right) {
                lowest_right = cost + r + 1 - p;
            }

            del.push_back(make_pair(p, cost));
        }
        ...
        auto it1 = poses.lower_bound(make_pair(l - 1, 0));
        if (it1 != poses.end() && it1->first == l - 1) {
            setmin(lowest_left, it1->second);
            poses.erase(it1);
        }

        auto it2 = poses.lower_bound(make_pair(r + 1, 0));
        if (it2 != poses.end() && it2->first == r + 1) {
            setmin(lowest_right, it2->second);
            poses.erase(it2);
        }

        if (l - 1 >= 1) {
            poses.insert(make_pair(l - 1, lowest_left));
        }
        if (r + 1 <= N) {
            poses.insert(make_pair(r + 1, lowest_right));
        }

    }

    int ans = INF;
    for (auto [p, cost]: poses) {
        setmin(ans, cost);
    }
